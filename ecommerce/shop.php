...
$brandResult = $brandObject->setStatus(1)->read();
if ($brandResult) {
...
$productData = $productObject->setStatus(1)->read();

if ($_GET) {
    if (isset($_GET['cat'])) {
        if (is_numeric($_GET['cat'])) {
            $categoryExists = $categoryObject->setId($_GET['cat'])->categoryExists();
            if ($categoryExists) {
                $productData = $productObject->setCategory_id($_GET['cat'])->getProductByCat();
            } else {
        ...
        if (is_numeric($_GET['sub'])) {
            $subcategoryExists = $subCategoryObject->setId($_GET['sub'])->subcategoryExists();
            if ($subcategoryExists) {
                $productData = $productObject->setSubcategory_id($_GET['sub'])->getProductBysub();
            } else {
                header("location:errors/404.php");
                die;
            }
        ...
        if (is_numeric($_GET['brand'])) {
            $brandExists = $brandObject->setId($_GET['brand'])->brandExists();
            if ($brandExists) {
                $productData = $productObject->setBrand_id($_GET['brand'])->getProductByBrand();
            } else {
                header("location:errors/404.php");
                die;
            }
                    ...
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            ...
                            <?php
                                ...
                                echo "<div class='alert alert-warning text-center m-5 col-4 '>No Any Product yet</div>";
                            }
                            ?>
                        </div>
                    </div>
                                ...
                                ?>
                                        <li> <a data-toggle="collapse" data-parent="#faq" href="#shop-catigory-<?= $category['id'] ?>"><?= $category['name_en'] ?> <i class="ion-ios-arrow-down"></i></a>
                                            <ul id="shop-catigory-<?= $category['id'] ?>" class="panel-collapse collapse ">
                                                <?php
                                                $subCategoryObject = new SupCategory;
                                                $subCategoryData = $subCategoryObject->setCategory_id($category['id'])->read();
                                                if ($subCategoryData) {
                    ...
                    <div class="shop-widget mt-40 shop-sidebar-border pt-35">
                        ...
                        <div class="sidebar-list-style mt-20">
                            <ul>
                                ...
                                ?>
                                        <li><input type="checkbox"><a href="shop.php?brand=<?= $brand['id'] ?>"><?= $brand['name_en'] ?> </a>
                                    <?php
                                    }
                                } else {
                                    echo "<div class='alert alert-warning'>No Any Brand yet</div>";
                                }
                                    ?>
                            </ul>
                        </div>
